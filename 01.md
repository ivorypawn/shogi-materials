...
I'll use modified Kitao–Kawasaki notation. The example line described here is read out for example, ... and so on.
This notation may be not so familiar to you. This is rather like the notation used in Japan and can be easily translated by Japanese players.
...
<!-- 
I think few people can properly understand what brinkmate is just by reading the description.
...
Here is the list of possible replies by white. So find each line to mate.
By the way, I suggest you review this list for each exercise that will appear later.
...
<!--
Here I'll suggest only the line after ☖金'22 and skip the rest.
...
I recommend you reproduce on your own board in case you were not sure.
...
(If had ☖桂 on 81, it would be brinkmate)
...
This position is NOT brinkmate. Not brinkmate, which means there is a way for white out from mating net.
...
White's turn.
Analyze each of the following states are brinkmate or not.
If not, calculate if there is a way to force brinkmate.
...
Note that ☗桂'24 can be effective when white does not have sufficient 金s in hand.
...
White's turn.
Analyze if each of the following states are brinkmate or not.
If not, calculate if there is a way to force brinkmate.
...
###
(By the way, I initially thought of ☗桂'75. If ☖歩 on 94 were on 93, ☗桂'75 would threat checkmate followed by ☗飛-82+!)
...
I created this position for my own purposes, but similar positions often appear in games between top professionals.
...
If white plays <span style="color: var(--hide-green);">☖桂'86?</span> here,

<div class="kifu">(*) ☗馬x32 ☖玉x ☗飛x21+ ☖玉x

☗銀'23 (Brinkmate).</div>

As for alternative <span style="color: var(--hide-green);">☖桂-65</span>, black can't choose <span style="color: var(--hide-green);">(*)</span> because

<div class="kifu">☖龍x78! ☗銀x ☖桂x57+! ...#</div></br>

So black will reply with <span style="color: var(--hide-green);">☗桂-45</span> 
or <span style="color: var(--hide-green);">☗馬x21</span> for example, then 
...
We have supposed in problems that every other pieces are in white's hand. 
However, in a game, lack of pieces in your opponent's hand is likely to occur and cause to change the king's state.

### Example
This 玉 will survive if there is 銀 or 角 in hand. Otherwise, it is brinkmate.
...
## 
And I recommend you try to make the position in your game!

---
##
Here is the preview of the basic brinkmate position of Part 2.
