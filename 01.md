...
---
このリストによると、☗桂'13の可否が影響するのは☖角'77の変化だけです。
-->
...
---
しかしexer.3.1 の局面で☖角'77とすると、☗金'32で詰みます。またほかの変化では、☖香-13の存在は影響がありません。
したがって、exer.3.1 の局面は必死です。

exer.3.2 の局面はどうでしょう。今度は ☖角'77 ☗金'32 に☖玉-11とする余地がありますが、黒はさらに☗桂'33と攻撃を続けられます。
白は21と22を同時にカバーしなければなりません。☖金'31のサクリファイスは、☗金x☖角x33+☗金'21#のため無効です。したがってここで☖角x33+は強制手ですが、☗金xとした局面が実は必死です。
ここで白が☖飛'92と最後の抵抗をしたときの詰み手順は検討しましたか？　もし未検討なら、再度動画を止めて考えてください。OK？　☗角'22 ☖玉-21 ☗銀x12+ ☖玉x ☗香'13 ☖玉-21 ☗香-11+で詰みです。
したがって、exer.3.2 の局面も最終的に必死にすることができます。
 -->
...
...
結論としてこの変化はドローになります。ただし実戦で白の持ち駒に金が十分ないときは、これでも必死になる可能性があります。
 -->
---
<!-- _class: small -->
## Promoted pieces
...
<!--
Exer. 4.1 の局面は必死です。☖馬-97 の存在は影響を及ぼしません。

Exer. 4.2 の局面は白に☖龍-52とされると必死がかかりません。すべての詰み筋がカバーされていることを確認してください。

Exer. 4.3 はやや複雑です。☗桂'13がブロックされているため、はじめは
Exer. 3.2 と同様に進みます。...
ここで、Exer. 3.2 では☗角'22から詰みました。この局面は詰みがありませんが、次の一手で必死になります。☗角'44。これで必死になっていることを確認してください。

なおもしもこの馬が35にあったら、このダイアゴナルはブロケードされます。したがって、結論も「必死はかからない」に変わります。
...
<!-- 
ここからはより実戦で起こり得そうなポジションについて考えていきます。
 -->
---
## From my own game
...
<!-- 
ステップ3の1問目は、私自身のゲームから抽出した必死問題です。ちなみにそのゲームは30秒の秒読みでした。私と同じ状況でクリアしたい人は、30秒の時間制限をつけてみるのもよいと思います。
3, 2.. では解答を示します。
 -->
---
## From my own game
...
<!-- 
これまでのコンテクストから、飛車のサクリファイスが見えた人は多いと思います。しかし直ちに☗飛x81+☖玉x☗銀'83としても☖金'82で必死がかかりません。
初手は桂のサクが正解です。白は82と62をカバーする必要がありますが、☖銀'71は☗金'62☖銀x☗飛-82+...#です。したがって☖歩xは強制手です。そこで☗飛x81+☖玉x☗銀'83とすれば、親しみのある必死のポジションが現れます。
 -->
